...
export function buildZomatoOrder(cart, coupon) {
    ...
    const items = filteredItems.map((item) => {
        ...
        if (
            ...
            addonTotal = item.addons.reduce((total, current) => {
    ...
    const subtotal = items.reduce(
        (total, current) => total + current.itemTotal,
        0,
    );
    ...
    let discount = 0;
    if (coupon && coupon.toUpperCase() === "FIRST50") {
        discount = Math.min(subtotal * 0.5, 150);
    } else if (coupon && coupon.toUpperCase() === "FLAT100") {
        discount = 100;
