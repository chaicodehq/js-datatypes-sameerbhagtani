...
export function processRailwayPNR(pnrData) {
    ...
    if (!pnr || typeof pnr !== "string" || !/^\d{10}$/.test(pnr)) return null;
    if (!train || typeof train !== "object" || train === null) return null;
    if (!Array.isArray(passengers) || passengers.length === 0) return null;

    const pnrFormatted =
        pnr.slice(0, 3) + "-" + pnr.slice(3, 6) + "-" + pnr.slice(6);

    const { number, name, from, to } = train;
    const trainInfo = `Train: ${number} - ${name} | ${from} → ${to} | Class: ${classBooked}`;

    const passengersData = pnrData.passengers.map((p) => {
    ...
    const confirmed = passengersData.filter(
        (p) => p.statusLabel === "CONFIRMED",
    ).length;
    const waiting = passengersData.filter(
        (p) => p.statusLabel === "WAITING",
    ).length;
    const cancelled = passengersData.filter(
        (p) => p.statusLabel === "CANCELLED",
    ).length;
    const rac = passengersData.filter((p) => p.statusLabel === "RAC").length;
    ...
    const summary = {
        ...
        cancelled,
        rac,
        allConfirmed: passengersData.every((p) => p.isConfirmed),
        anyWaiting: passengersData.some((p) => p.statusLabel === "WAITING"),
    };

    const nonCancelled = passengersData.filter(
        (p) => p.statusLabel !== "CANCELLED",
    );
